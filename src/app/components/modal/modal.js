...
/** @typedef {import('../../types').CardCenterCoords} CardCenterCoords */
...
export default class Modal extends BaseComponent {
  ...
  constructor(state) {
    ...
    this.image = this.createImage(gift);

    const headerComponents = this.createHeader(gift);
    this.header = headerComponents.header;
    this.category = headerComponents.category;
    this.name = headerComponents.name;
    this.description = headerComponents.description;

    const footerComponents = this.createFooter(gift);
    this.footer = footerComponents.footer;
    this.ratingBlock = footerComponents.ratingBlock;

    const container = tag.div({ classList: styles.modalContainer }, [
      this.image,
      tag.div({ classList: styles.modalContent }, [this.header, this.footer]),
    ]);
  ...
  createImage(gift) {
    return tag.img({
      ...
      src: relatedData[gift?.category ?? 'For Work'].imgSrc,
      alt: relatedData[gift?.category ?? 'For Work'].imgAlt,
    });
  }
   ...
   * @param {Gift | null} gift
   * @returns {{ header: BaseComponent, category: BaseComponent, name: BaseComponent, description: BaseComponent }}
   */
  createHeader(gift) {
    const category = tag.div({
      classList: [
        'heading-4',
        relatedData[gift?.category ?? 'For Work'].captionClass,
      ],
      text: gift?.category ?? '',
    });
    const name = tag.div({
    ...
    const header = tag.div({ classList: styles.modalHeader }, [
      category,
      name,
      description,
    ]);
   ...
   * @param {Gift | null} gift
   * @returns {{footer: BaseComponent, ratingBlock: BaseComponent}}
   */
  createFooter(gift) {
  ...
  createRatingLines(gift) {
    return Object.entries(gift.superpowers).map(([name, score]) => {
      const iconsBlock = this.createIconsBlock(
        Number(
          gift.superpowers[
            /** @type {keyof typeof gift.superpowers} */ (name)
          ][1]
        )
      );
  ...
  fillModalData(gift) {
    ...
    /** @type {HTMLImageElement} */ (this.image.element).src =
      relatedData[gift.category].imgSrc;
    /** @type {HTMLImageElement} */ (this.image.element).alt =
      relatedData[gift.category].imgAlt;
