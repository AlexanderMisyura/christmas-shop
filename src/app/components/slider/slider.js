...
function debounce(func, timeout) {
  /** @type {NodeJS.Timeout} */
  let timeoutId;
  return function wrapperDebounced() {
...
export default class Slider extends BaseComponent {
  ...
  constructor(state) {
    ...
    const sliderParts = this.createSliderParts();
    this.slider = sliderParts.slider;
    this.left = sliderParts.left;
    this.right = sliderParts.right;
    this.fillSlider();
   ...
   * @returns {{slider: BaseComponent, left: SliderControl, right: SliderControl}}
   */
  createSliderParts() {
    ...
    const slider = tag.div({ classList: styles.sliderRowContent });
    ...
    return { slider, left: controls.left, right: controls.right };
  }
  ...
  createControlBlock() {
    ...
    left.button.addListener('click', left.handler);
    right.button.addListener('click', right.handler);

    const buttonsBlock = tag.div({ classList: styles.sliderRowButtonsBlock });
    buttonsBlock.appendChildElements([left.button, right.button]);

    return { buttonsBlock, left, right };
  }
